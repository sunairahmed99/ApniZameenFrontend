...
import { FaMapMarkerAlt, FaChevronLeft, FaChevronRight, FaPhoneAlt } from 'react-icons/fa';
import { useAgenciesList } from '../../hooks/usePublicFeatures';
import { useLocations } from '../../hooks/useLocations';
import './FeaturedAgencies.css';
import './CityTabs.css';
...
const FeaturedAgencies = () => {
  ...
  const handlePageChange = (pageNumber) => {
    ...
    const section = document.querySelector('.featured-agencies-section');
    if (section) {
  ...
  return (
    <div className="featured-agencies-section py-4">
      <div className="container">
        <h3 className="mb-4 fw-bold">Featured Agencies</h3>

        {/* City Tabs Slider */}
        <div className="project-city-tabs-container mb-4">
          <button className="city-nav-btn left" onClick={() => {
            const el = document.querySelector('.featured-agencies-section .city-tabs-scroll-wrapper');
          ...
          <div className="position-relative">
            <div className="row g-4">
              {paginatedAgencies.map((agent) => (
                <div className="col-lg-3 col-md-6 col-6" key={agent._id}>
                  <div
                    className="featured-agency-card bg-white p-3 rounded h-100 d-flex align-items-center"
                    onClick={() => navigate(`/agency/${agent._id}`)}
                    style={{ cursor: 'pointer' }}
                  >
                    <div className="featured-logo-wrapper me-3">
                      <OptimizedImage
                    ...
                    <div className="flex-grow-1">
                      <h6 className="fw-bold mb-1" style={{ fontSize: '0.95rem', lineHeight: '1.2', wordWrap: 'break-word' }} title={agent.name}>{agent.name}</h6>
                      <div className="text-muted small mb-1 text-truncate">
                        {agent.forSale || 0} for Sale | {agent.forRent || 0} for Rent
                      </div>
            ...
            {agencies.length === 0 && (
              <div className="alert alert-light text-center border shadow-sm py-4">
                <p className="mb-0 text-muted">No Featured Agencies found in {selectedCity}.</p>
              </div>
            )}
