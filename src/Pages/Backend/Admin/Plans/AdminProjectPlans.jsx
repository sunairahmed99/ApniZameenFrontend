...
import { useProjectPlans } from '../../../../hooks/useAdminFinancials';
import './AdminPlans.css'; // Reusing existing CSS

import DataTable from '../../../../Components/Backend/Admin/DataTable.jsx';

const AdminProjectPlans = () => {
    const { plans, isLoading: loading, createPlan, updatePlan, deletePlan } = useProjectPlans();
    ...
    const handleDelete = async (id) => {
        if (!confirm('Delete project plan?')) return;
        try {
            await deletePlan(id);
        } catch (err) {  }
    };
    ...
    return (
        <div className="p-4">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <h2 className="mb-0">Project Plans Management</h2>
                <button className="btn btn-primary" onClick={() => setShowForm(!showForm)}>
                    {showForm ? 'Cancel' : <><FaPlus className="me-2" /> Create Project Plan</>}
                </button>
            </div>
...
export default AdminProjectPlans;
